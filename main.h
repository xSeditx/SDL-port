...
class CREATURE{
       public:
		   ...
		   RGBA_COLOR COLOR;
        	 ...
        	 CREATURE::CREATURE();
        	 CREATURE::CREATURE(int parent, int x,int y, int speed,float angle,bool sex,RGBA_COLOR color,int FOV,int SIGHT_DISTANCE);
...
CREATURE::CREATURE(){
	ANGLE = SPEED = HEALTH = FOV = SIGHT_DISTANCE = AGE = NULL;
};
...
CREATURE::CREATURE(int parent, int x,int y, int speed,float angle,bool sex, RGBA_COLOR color,int fov,int sight){
	EXIST = FALSE;
	 ...
	 for(int SEES_OBJECT = 0;SEES_OBJECT < NUM_NEURONS;SEES_OBJECT++){
		 for(int RESPONSE = 0;RESPONSE < NUM_RESPONSES;RESPONSE++){
			 ...
			 if (parent != 0){
				 ...
				 BRAIN[SEES_OBJECT][RESPONSE] = rand() % 100000; 
			 }
		 }
	 }
...
void CREATURE::ROTATE(float CHANGE){ANGLE = _WRAP_ANGLE(ANGLE + CHANGE);}
	...
	float ANG;
	 ...
	 float Theta;
    	...
    	for(float  DIST = 2; DIST < (float)SIGHT_DISTANCE; DIST++ ){
    	    ...
    	    ANG = _WRAP_ANGLE(ANGLE + A);
			XX = x1 + DIST * cos(ANG * _PI / 180);
			YY = y1 + DIST * sin(ANG * _PI / 180);
			XX = WRAPX(XX);
			YY = WRAPY(YY);
	
			if(SHOW_VISION == TRUE) SDL_RenderDrawPoint(RENDER_HND,XX, YY);//dbDot(XX, YY, dbRGB(70,70,70));;
...
// I currently believe this to be a HUGE bottle neck of my program and feel there has to be a better way to handle it.
 ...
 int NX=0,NY=0;
		if(NUM_CREATURES >= MAX_CREATURES)return 0;
		   ...
		   if (WORLD[NX][NY] ==0 )
		   {
			   ...
			   int colormr = 2;
           RGBA_COLOR newcol = COLOR ;

	 	   CNEW = CREATURE(ID, NX, NY, rand()%6, rand()%360, rand() % 2, COLOR , FOV + (rand()%3-1), SIGHT_DISTANCE + (rand()%3-1));
...
void CREATURE::KILL(){
	...
	X = Y =  ANGLE = SPEED = HEALTH = FOV = SIGHT_DISTANCE = AGE = 0;
...
void CREATURE::DRAW(){
	// SDL_RenderDrawPoint(RENDER_HND, X, Y);//dbDot(X,Y,COLOR);
	SDL_SetRenderDrawColor(RENDER_HND, COLOR.R,COLOR.G, COLOR.B, 255);   ///dbInk(dbRGB(0,0,255),dbRGB(0,0,0));
...
/*
	...
	for(float A = ANGLE;A < ANGLE + 180;A+=40){
		XX = X + DIST * cos(_WRAP_ANGLE(A) * 3.14159 / 180);
		YY = Y + DIST * sin(_WRAP_ANGLE(A) * 3.14159 / 180);
